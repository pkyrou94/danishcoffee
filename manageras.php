...
try {
    ...
    $stmt = $conn->prepare("SELECT tiropita, hortopita, kuluri, tost, cake FROM apothemataas"); 
    $stmt->execute();
...
try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT ellinikos , frape , espresso , cappuccino , filtercoffee , tiropita , koulouri , tost , cake , xortopita , dieuthinsi , total , dianomeas FROM paragkeliesas"); 
    $stmt->execute();
