...
<div class="tagline-upper text-center text-heading text-shadow text-white mt-5 d-none d-lg-block">Danish Coffee Shops</div>
   ...
   <nav class="navbar sticky-top navbar-expand-lg navbar-light bg-faded py-lg-4">
	...
	<div class="bg-faded p-4 my-4">
		...
		<form action="updatepg.php" method="POST">
...
try {
    ...
    $stmt = $conn->prepare("SELECT tiropita ,kuluri, tost, cake , hortpita FROM apothematapg"); 
    $stmt->execute();
...
try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT ellinikos , frape , espresso , cappuccino , filtercoffee , tiropita , koulouri , tost , cake , xortopita , dieuthinsi , total , dianomeas FROM paragkeliespg"); 
    $stmt->execute();
